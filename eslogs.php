...
<title>RW Project - Наверное это самый активный список.</title>
...
else
{
	...
	<div class="panel-heading">Заходы\Выходы</div>
	...
	echo ('
		<form method="get" class="navbar-form navbar-right" role="search">
 		<div class="form-group">
  		...
  		<p>*Искать можно по Нику игрока или по IP</p>
		</form>
		</div>
	...
	@include ("scripts/bd.php");
		...
		if(!empty($_GET['search_adm'])) 
    	{
    		...
    		$result = mysql_query("SELECT * FROM EnterLog WHERE `Name` LIKE '%".$_GET['search_adm']."%' OR `TEXT` LIKE '%".$_GET['search_adm']."%' ORDER BY Date DESC");
    	}
    	else
    	{
    		$result = mysql_query("SELECT * FROM EnterLog ORDER BY Date DESC LIMIT 30000");
    	}
