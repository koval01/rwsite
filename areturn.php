...
<title>RW Project - Сейчас посмотрим...</title>
...
else
{
	...
	<div class="panel-heading">Заявки на восстановление лута</div>
	...
	echo ('
		<form method="get" class="navbar-form navbar-right" role="search">
 		<div class="form-group">
  		...
  		<p>*Искать можно по Нику и номеру заявки</p>
		</form>
		</div>
	...
	echo('<table class="table"><tr><th>ID</th><th>Игрок</th><th>Номер заявки</th></tr>');
	
	if(!empty($_GET['search_adm'])) 
	{
		$search = mysql_real_escape_string($_GET['search_adm']);
		$result = mysql_query("SELECT * FROM returnloot WHERE `Name` LIKE '%".$search."%' OR `Code` LIKE '%".$search."%' ORDER BY ID DESC");
	}
	else
	{
		$result = mysql_query("SELECT * FROM returnloot ORDER BY ID DESC");
	}
