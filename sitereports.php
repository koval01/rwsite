...
<title>RW Project - Всё же хорошо... Наверное наш веб-разработчик опять накосячил.</title>
...
else
{
	...
	echo ('
		<form method="get" class="navbar-form navbar-right" role="search">
 		<div class="form-group">
  		...
  		<p>*Искать можно по Нику игрока или по ID сообщения</p>
		</form>
		</div>
	...
	echo('<table class="table"><tr><th>ID</th><th>Дата</th><th>Ник игрока</th><th>Отзыв</th></tr>');
	
	if(!empty($_GET['search_adm'])) 
	{
		$search = mysql_real_escape_string($_GET['search_adm']);
		$result = mysql_query("SELECT * FROM RateSiteRW WHERE `Name` LIKE '%".$search."%' OR `ID` LIKE '%".$_GET['search_adm']."%' ORDER BY Date DESC");
	}
	else
	{
		$result = mysql_query("SELECT * FROM RateSiteRW ORDER BY Date DESC");
	}
