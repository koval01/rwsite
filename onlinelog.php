...
<title>RW Project - Живёт сам по себе.</title>
...
else
{
	...
	<div class="panel-heading">Статистика онлайна RW Project</div>
	...
	echo('<table class="table"><tr><th>Дата</th><th>Онлайн</th></tr>');
	
	if(!empty($_GET['search_adm'])) 
	{
		$search = mysql_real_escape_string($_GET['search_adm']);
		$result = mysql_query("SELECT * FROM ONLINE WHERE `Date` LIKE '%".$search."%' ORDER BY Date DESC");
	}
	else
	{
		$result = mysql_query("SELECT * FROM ONLINE ORDER BY Date DESC");
	}
