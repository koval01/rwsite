...
<title>RW Project - Сейчас мы всё узнаем.</title>
...
else
{
	...
	<div class="panel-heading">Админ-Лог</div>
	...
	echo('<table class="table"><tr><th>Ситуация</th><th>Дата</th><th>Ник администратора</th><th>Ник игрока</th><th>Описание</th></tr>');
	
	if(!empty($_GET['search_adm'])) 
	{
		$search = mysql_real_escape_string($_GET['search_adm']);
		$result = mysql_query("SELECT * FROM AdminLog WHERE `Name` LIKE '%".$search."%' OR `TargetNAME` LIKE '%".$_GET['search_adm']."%' ORDER BY Date DESC");
	}
	else
	{
		$result = mysql_query("SELECT * FROM AdminLog ORDER BY Date DESC");
	}
