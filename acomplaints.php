...
<title>RW Project - Жалобы игроков</title>
...
else
{
	...
	echo ('
		<form method="get" class="navbar-form navbar-right" role="search">
 		<div class="form-group">
  		...
  		<p>*Искать можно по Нику игрока или содержимому</p>
		</form>
		</div>
	...
	echo('<table class="table"><tr><th>Ник игрока</th><th>Дата</th><th>Содержимое</th><th>Действие</th></tr>');
	
	if(!empty($_GET['search_adm'])) 
	{
		$search = mysql_real_escape_string($_GET['search_adm']);
		$result = mysql_query("SELECT * FROM RWSiteComp WHERE `Name` LIKE '%".$search."%' OR `TEXT` LIKE '%".$_GET['search_adm']."%' ORDER BY Date DESC");
	}
	else
	{
		$result = mysql_query("SELECT * FROM RWSiteComp ORDER BY Date DESC");
	}
	...
	do
	{
		...
		if($qwr != null){
			...
			echo("</td><td>");
			if(strlen((string)$qwr['TEXT']) >= 70)
            {
                $TextReplace1 = mb_strimwidth ((string)$qwr['TEXT'], 0, 70);
                echo("<center><span class='text-st-gr'>".$TextReplace1."...</span></center>"); 
            }
            else
            {
                if((string)$qwr['TEXT'] == Null) echo("<center><span class='text-st-gr'>Отсутствует .</span></center>");
                if((string)$qwr['TEXT'] != Null) echo("<center><span class='text-st-gr'>".(string)$qwr['TEXT']."</span></center>");  
            }
			echo("</td><td>");
			echo("Недоступно");
			echo("</tr>");
